...
//marcador de tempo:1000 -> 1 seg
setInterval(function () {
    tempo++;
    document.querySelector("#tempo").innerHTML = "<b>" + tempo + "</b>";
}, 1000);
...
var Tamagoshi = {
  ...
  // Time passes
    ...
    timePasses: function (intervalID) {
      ...
      this.setTamagoshiMeters(intervalID);
    },
  
    setTamagoshiMeters: function (intervalID) {
      ...
      if (!this.isAlive()) {
        ...
        $(".alert-msg").text("Too late! Your Tamagoshi is DEAD!");
        ...
        $(".alert-msg").text("Your Tamagoshi is sick: Feed, play, put to bed, give medicine!")
      } else if (this.foodLevelWarning()) {
        $(".alert").addClass("alert-warning");
        $(".alert-msg").text("Your Tamagoshi is hungry: Feed it!");
      } else if (this.happinessLevelWarning()) {
        $(".alert").addClass("alert-warning");
        $(".alert-msg").text("Your Tamagoshi is sad: Play with it!");
      } else if (this.restedLevelWarning()) {
        $(".alert").addClass("alert-warning");
        $(".alert-msg").text("Your Tamagoshi is tired: Put it to bed!")
      };
    }
  }; // end Tamagoshi
  ...
  $(document).ready (function () {
    ...
    $("form#form-to-disappear").submit(function(event){
      ...
      var inputtedName = $("input#Tamagoshi-name").val();
      var inputtedBirthday = $("input#Tamagoshi-birthday").val();
  
      $(".Tamagoshi-name").text(inputtedName);
      ...
      $(".Tamagoshi-birthday").text(dateToDisplay);
  
      $("input#Tamagoshi-name").val("");
      $("input#Tamagoshi-birthday").val("");
      ...
      var mytamagoshi = Object.create(Tamagoshi);
      myTamagoshi.initialize(inputtedName, numberImage, dateToDisplay);
  
      myTamagoshi.setTamagoshiMeters(0);
      $("#meter-area").show();
  
      var intervalID;
  
      intervalID = setInterval(function() {
        myTamagoshi.timePasses();
        if (!myTamagoshi.isAlive()) {
          clearInterval(intervalID);
        };
      }, difficulty);
      ...
      $("button#feed").click (function () {
        clearInterval(intervalID);
        myTamagoshi.setTamagoshiMeters();
        if (myTamagoshi.isSleeping) {
          myTamagoshi.isSleeping = false;
        };
        myTamagoshi.feed();
        myTamagoshi.setTamagoshiMeters();
        if (myTamagoshi.isAlive) {
